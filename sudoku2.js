...
let checkMe = function(){
    for(let i=0; i<9; i++){
        for(let j=0; j<9; j++){
            if(unsolved[i][j] !== solved[i][j]){
                alert("Sorry, you didn't solve this yet!");
                return false;
            }
        }
    }
    alert("yaay!  You solved it");
    return true; //all of them matched.
}

let unsolved=function userGrid(grd){
...
for(let i=0; i<9; i++){
    for(let j=0; j<9; j++){
        ...
        document.getElementById(s).innerHTML = grid[i][j];
            ...
            document.getElementById(s).innerHTML = '<input type="text" style="height:40px;width:40px;font-size:18pt;" id="s" min="1" max="9"></>';
        }
    }
}

function dealWithNumber(e, elementIdNo){
   /* let x = document.getElementById(elementIdNo).value; 
    alert(x);*/
    /*let input = document.getElementById(elementIdNo).value;
    alert(input);*/
    /*alert(e.keyCode);
    alert(elementIdNo); */
    let i=elementIdNo[3];
    ...
    switch(e.keyCode) {
        ...
        case 50:
         ...
         console.log(unsolved);
         break;
        default: alert("You must enter a number between 1 and 9");
        document.getElementById(elementIdNo).innerHTML = '<input type="text"   style="height:40px;width:40px;font-size:18pt;" id="s" min="1" max="9"></>';  //to delete wrong inputs
        break;
    // code block
}
}
